    ...
    <div className={className}>
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Enhance your rental with specialized attachments ({rentalDays} {rentalDays === 1 ? 'day' : 'days'})
        </p>
      </div>
                      ...
                      <div className="text-xs text-gray-500 mt-2">
                        {typeof attachment.specifications === 'object' && (
                          <div className="space-y-0.5">
                            {Object.entries(attachment.specifications as Record<string, any>).slice(0, 2).map(([key, value]) => (
                              <div key={key}>
      ...
      {selected.length > 0 && (
        <div className="mt-6 bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg p-5 border-2 border-orange-200 shadow-sm">
          ...
          <div className="space-y-2 mb-3">
            {selected.map(att => (
              <div key={att.id} className="flex justify-between items-center text-sm bg-white rounded px-3 py-2">
                <span className="text-gray-700">{att.name}</span>
                <div className="flex items-center gap-3">
                  <span className="text-gray-500">
                    ${att.daily_rate.toFixed(2)}/day × {att.days_rented} {att.days_rented === 1 ? 'day' : 'days'}
                  </span>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-800">
                Add specialized attachments to handle any job! Popular choices: Hydraulic Auger for post holes,
                Pallet Forks for material handling, or Hydraulic Breaker for concrete work.
              </p>
            </div>
          </div>
        </div>
