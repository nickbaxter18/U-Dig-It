      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <div className="text-sm text-blue-800 space-y-1">
          {equipment.hourly_rate && (
            <div>• <strong>4-hour job?</strong> Hourly rate = ${(equipment.hourly_rate * 4).toFixed(2)}</div>
          )}
          {equipment.half_day_rate && equipment.hourly_rate && (
            <div>• <strong>6-hour job?</strong> Half-day saves ${((equipment.hourly_rate * 6) - equipment.half_day_rate).toFixed(2)} vs hourly!</div>
          )}
          {equipment.weeklyRate && equipment.dailyRate && (
            <div>• <strong>7+ days?</strong> Weekly saves ${((equipment.dailyRate * 7) - equipment.weeklyRate).toFixed(2)} vs daily!</div>
          )}
          {equipment.monthlyRate && equipment.dailyRate && (
            <div>• <strong>30+ days?</strong> Monthly saves ${((equipment.dailyRate * 30) - equipment.monthlyRate).toFixed(2)} vs daily!</div>
          )}
        </div>
      </div>
        ...
        <div className="mt-4 p-4 bg-green-50 border-2 border-green-200 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-lg font-bold text-gray-900">{selected.label} - ${selected.total.toFixed(2)}</div>
            </div>
