...
export function SupabaseAuthProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (error) {
        if (process.env.NODE_ENV === 'development') {
          logger.debug('[SupabaseAuthProvider] Failed to fetch role from users table, falling back to metadata', {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        if (error) {
          logger.error('[SupabaseAuthProvider] Error getting session:', {
            component: 'SupabaseAuthProvider',
            action: 'error',
          ...
          if (process.env.NODE_ENV === 'development') {
            logger.debug(
              ...
              { component: 'SupabaseAuthProvider', action: 'debug', metadata: { email: session.user.email } }
            );
          ...
          try {
            ...
            if (cookieSession) {
              ...
              logger.error(
                ...
                if (process.env.NODE_ENV === 'development') {
                  logger.debug(
                    '[SupabaseAuthProvider] Session restored from cookie:',
                    { component: 'SupabaseAuthProvider', action: 'debug', metadata: { email: newSession.user.email } }
                  );
                }
                setUser(newSession.user);
                setRole(newSession.user.user_metadata?.role ?? null);
                void fetchAndSetUserRole(newSession.user.id, newSession.user.user_metadata?.role ?? null);
              } else {
                if (process.env.NODE_ENV === 'development') {
            ...
            logger.error('[SupabaseAuthProvider] Error reading session from cookie:', {
              component: 'SupabaseAuthProvider',
              action: 'error',
        ...
        logger.error('[SupabaseAuthProvider] Failed to initialize auth:', {
          component: 'SupabaseAuthProvider',
          action: 'error',
        }, error instanceof Error ? error : new Error(String(error)));
        if (mounted) {
  ...
  useEffect(() => {
    if (user?.id) {
      void fetchAndSetUserRole(user.id, user.user_metadata?.role ?? null);
    }
  }, [user?.id]);
