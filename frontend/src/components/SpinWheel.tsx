...
const WHEEL_SLICES = [
  ...
  { id: 'try_again', label: 'Try Again', color: '#6B7280', gradient: 'from-gray-400 to-gray-600', weight: 1 },
  { id: '75', label: '$75', color: '#F59E0B', gradient: 'from-yellow-400 to-orange-500', weight: 1 },
  { id: 'try_again', label: 'Try Again', color: '#6B7280', gradient: 'from-gray-400 to-gray-600', weight: 1 },
  { id: '50', label: '$50', color: '#10B981', gradient: 'from-green-400 to-green-600', weight: 1 },
  { id: 'try_again', label: 'Try Again', color: '#6B7280', gradient: 'from-gray-400 to-gray-600', weight: 1 },
...
export default function SpinWheel({ isOpen, onClose, onWin }: SpinWheelProps) {
  ...
  const getAudioContext = () => {
    if (!audioContextRef.current) {
      try {
        ...
        logger.error('Failed to create AudioContext', {
          component: 'SpinWheel',
  ...
  const playWinSound = () => {
    try {
      ...
      const melody = [
        ...
        { freq: 1046.50, time: 0.24 }    // C6 (octave up - triumphant!)
      ];
      ...
      bassGain.gain.setValueAtTime(0.20, now + 0.24);
  ...
  const playLoseSound = () => {
    try {
      ...
      logger.error('Lose sound failed', {
        component: 'SpinWheel',
  ...
  const playTickSound = () => {
    try {
      ...
      midGain.gain.setValueAtTime(0.10, audioContext.currentTime);
      ...
      logger.error('Tick sound failed', {
        component: 'SpinWheel',
  ...
  const scheduleTicksForSpin = (startRotation: number, endRotation: number, spinNumber: number) => {
    ...
    const findTimeForRotation = (rotationProgress: number): number => {
      ...
      for (let i = 0; i < 30; i++) { // Increased iterations for better accuracy
        mid = (low + high) / 2;
    ...
    if (!isOpen) return;
      ...
      const createSession = async () => {
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Failed to start spin session';
    ...
    try {
      ...
      if (wheelRef.current) {
        ...
        if (data.sliceIndex !== undefined) {
          ...
          const matchingSlices = WHEEL_SLICES
            .map((slice: any, index: any) => ({ slice, index }))
      ...
      logger.error('[SpinWheel] Spin failed', {
        component: 'SpinWheel',
        action: 'spin_error',
      }, error as Error);
    }
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[min(95vw,28rem)] max-h-[96vh] rounded-2xl bg-white flex flex-col overflow-hidden animate-[modal-fade-in_0.3s_ease-out]"
           style={{
             boxShadow: '0 0 50px rgba(59, 130, 246, 0.2), 0 20px 40px rgba(0, 0, 0, 0.3)',
             border: '1px solid rgba(229, 231, 235, 1)'
           }}>
        {/* Enhanced Header with subtle depth - Compact */}
        <div className="relative flex-shrink-0 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 p-3 text-white"
             style={{ boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1), inset 0 1px 1px rgba(255, 255, 255, 0.1)' }}>
          <div className="absolute inset-0 bg-black bg-opacity-10"></div>
          <div className="relative flex items-center justify-between">
            <div>
              <h2 id="spin-wheel-title" className="text-xl font-bold" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>
                {hasWon ? '🎉 Congratulations!' : isLastSpin ? '🎯 Final Spin!' : '🎰 Spin to Win!'}
              </h2>
              <p id="spin-wheel-description" className="text-blue-100 mt-0.5 text-sm" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.15)' }}>
                {hasWon
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-3">
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3" role="alert">
              <div className="flex items-center">
                <svg className="h-6 w-6 text-red-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <form onSubmit={handleGuestSubmit} className="space-y-3">
                ...
                <div className="flex items-start space-x-2">
                  <input
                    ...
                    onChange={(e: any) => setMarketingConsent(e.target.checked)}
                    className="mt-1 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <label htmlFor="marketing-consent" className="text-xs text-gray-600">
                    I'd like to receive occasional deals and equipment tips (you can unsubscribe anytime) <span className="text-red-500">*</span>
                  </label>
                </div>
          ...
          <>
          <div className="relative mx-auto w-full max-w-[256px]" style={{ aspectRatio: '1/1', maxHeight: '256px' }}>
            {/* Enhanced Outer Ring with subtle glow */}
            <div className="absolute inset-0 rounded-full bg-gradient-to-br from-blue-50 via-indigo-100 to-purple-100 border-4 border-white"
                 style={{
            ...
            >
              ...
              >
                ...
                {(() => {
                  ...
                  const totalWeight = WHEEL_SLICES.reduce((sum: any, slice: any) => sum + slice.weight, 0);
                  const radius = 180;
                  const centerX = 200;
                  const centerY = 200;
                  let currentAngle = 0;

                  return WHEEL_SLICES.map((slice: any, index: any) => {
                    ...
                    const markerX = centerX + markerRadius * Math.cos(textAngle * Math.PI / 180);
                    const markerY = centerY + markerRadius * Math.sin(textAngle * Math.PI / 180);
                      ...
                      <g key={index}>
                        ...
                        {slice.label === 'Try Again' ? (
                          <text
                            ...
                            strokeWidth="0.5"
                          >
                            <tspan x={textX} dy="-8">Try</tspan>
                            <tspan x={textX} dy="18">Again</tspan>
                          </text>
                        ) : (
                        <text
                          x={textX}
                        ...
                        <circle cx={markerX} cy={markerY} r="5" fill="cyan" stroke="blue" strokeWidth="2" />
                      </g>
                    );
                  });
                })()}
                ...
                <defs>
                  {WHEEL_SLICES.map((slice: any, index: any) => (
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-16 h-16 rounded-full border-4 border-white flex items-center justify-center z-20"
                 style={{
                   background: 'linear-gradient(135deg, #fbbf24 0%, #f59e0b 70%, #ea580c 100%)',
                   boxShadow: '0 4px 12px rgba(0, 0, 0, 0.15), inset 0 2px 4px rgba(255, 255, 255, 0.3), inset 0 -2px 4px rgba(0, 0, 0, 0.2)'
                 }}>
            ...
            <div className="absolute top-0 z-30"
                 style={{
                   ...
                   filter: 'drop-shadow(0 6px 16px rgba(220, 38, 38, 0.6)) drop-shadow(0 2px 8px rgba(0, 0, 0, 0.3))'
                 }}>
              <svg width="14" height="38" viewBox="0 0 14 38" style={{ display: 'block' }}>
                <defs>
                  ...
                  <linearGradient id="pointer-red-gradient" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stopColor="#fca5a5" stopOpacity="1" /> {/* Light red top */}
                    <stop offset="20%" stopColor="#f87171" stopOpacity="1" /> {/* Bright red */}
                    <stop offset="60%" stopColor="#dc2626" stopOpacity="1" /> {/* Bold red */}
                    <stop offset="100%" stopColor="#991b1b" stopOpacity="1" /> {/* Deep red tip */}
                  </linearGradient>
          ...
          {session.coupon_code && timeLeft > 0 && (
            <div className="mt-1 text-center">
              <p className="text-xs text-gray-600">Expires in: <span className="font-mono font-bold text-red-600">{formatTime(timeLeft)}</span></p>
            </div>
          )}
          ...
          {spinResult && session && (
            <div className="mt-2 text-center" role="status" aria-live="polite">
              {spinResult === 'try_again' ? (
                <div className="bg-gradient-to-r from-orange-50 to-yellow-50 border border-orange-200 rounded-xl p-2">
                  ...
                  {session.spins_used === 1 ? (
                    ...
                    <div className="text-orange-700">
                      ...
                      <p className="text-sm">Big prizes are waiting... will it be your lucky one?</p>
                </div>
              ) : (
                ...
                <div className="bg-gradient-to-br from-green-50 via-emerald-50 to-green-50 border-2 border-green-300 rounded-lg p-2"
                     style={{ boxShadow: '0 4px 12px rgba(16, 185, 129, 0.15)' }}>
                  <div className="text-2xl mb-1" aria-hidden="true">🎉</div>
                  <p className="text-base font-bold text-green-600 mb-0.5" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.08)' }}>
                    Congratulations!
                  </p>
                  <p className="text-sm font-bold text-green-700 mb-1">You won ${spinResult} off!</p>
                  <div className="bg-white border-2 border-green-400 rounded-lg p-1.5 mb-1 relative"
                       style={{ boxShadow: '0 2px 8px rgba(16, 185, 129, 0.1), inset 0 1px 2px rgba(255, 255, 255, 0.5)' }}>
                    <p className="text-[10px] text-gray-600 mb-0.5 text-center">Your Code:</p>
                    <div className="flex items-center justify-center">
                      <p className="text-lg font-mono font-bold text-green-600 tracking-wider text-center"
                  ...
                  <p className="text-green-700 font-semibold text-xs">💾 Don't forget to save your code!</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="mt-2 text-center">
            {hasWon ? (
              ...
              <button
                  ...
                  style={{
                    boxShadow: '0 4px 12px rgba(16, 185, 129, 0.3), 0 0 20px rgba(16, 185, 129, 0.1)',
