...
const serviceAreas: ServiceArea[] = [
  ...
  { name: 'Hampton', slug: 'hampton', description: 'Agricultural & acreage properties' },
  {
    ...
    description: 'Waterfront estates & rural',
  },
...
export default function ServiceAreaLinks({
  ...
  let areas = currentSlug ? serviceAreas.filter((area) => area.slug !== currentSlug) : serviceAreas;
...
export function NearbyServiceAreas({ currentSlug }: { currentSlug: string }) {
  ...
  return (
    <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-6">
      <h3 className="mb-4 text-lg font-bold text-gray-900">🗺️ Nearby Communities We Serve</h3>
      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
        {nearbyAreas.map((area: unknown) => (
          <Link
            key={area.slug}
            href={`/service-areas/${area.slug}`}
