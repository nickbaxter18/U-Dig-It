...
export async function PATCH(
  ...
  try {
    ...
    if (payload.durationHours !== undefined) updateBody.duration_hours = payload.durationHours ?? null;
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Unable to update maintenance log' },
        { status: 500 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (deleteError) {
      ...
      return NextResponse.json(
        { error: 'Unable to delete maintenance log' },
        { status: 500 }
      );
    }
