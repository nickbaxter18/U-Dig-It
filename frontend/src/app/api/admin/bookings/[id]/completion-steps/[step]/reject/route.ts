  ...
  async (request: NextRequest, { params }: { params: { id: string; step: string } }) => {
    try {
      ...
      switch (step) {
        case 'insurance_uploaded': {
          ...
          const { data: updatedDoc, error: updateError } = await supabaseAdmin
            ...
            .update({
              ...
              reviewNotes: `${validated.reason}${validated.notes ? `\n\n${validated.notes}` : ''}`,
            })
          ...
          if (updateError) {
            logger.error(
              ...
              {
                component: 'admin-step-reject-api',
                action: 'insurance_reject_error',
                metadata: { documentId: insuranceDoc.id },
              },
              updateError
            );
            return NextResponse.json({ error: 'Failed to reject insurance document' }, { status: 500 });
          }
        ...
        case 'license_uploaded': {
          ...
          if (updateError) {
            logger.error(
              ...
              {
                component: 'admin-step-reject-api',
                action: 'verification_reject_error',
                metadata: { requestId: verification.id },
              },
              updateError
            );
            return NextResponse.json({ error: 'Failed to reject ID verification' }, { status: 500 });
          }
        ...
        default:
          return NextResponse.json({ error: 'Invalid step type for rejection' }, { status: 400 });
      }

      logger.info('Step rejected successfully', {
        component: 'admin-step-reject-api',
        action: 'step_rejected',
        metadata: { bookingId, step, adminId: adminUser.id, reason: validated.reason },
      });
      ...
      logger.error(
        'Unexpected error rejecting step',
        {
          component: 'admin-step-reject-api',
