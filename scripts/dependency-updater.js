  ...
  async checkOutdatedDependencies() {
    ...
    try {
      ...
      if (Object.keys(outdatedData).length > 0) {
        console.log(`📦 Found ${Object.keys(outdatedData).length} outdated dependencies`);
        this.updateLog.push(`Found ${Object.keys(outdatedData).length} outdated dependencies`);
      } else {
  ...
  async checkSecurityVulnerabilities() {
    ...
    try {
      ...
      if (auditData.vulnerabilities && Object.keys(auditData.vulnerabilities).length > 0) {
        ...
        this.securityIssues.push(`Found ${vulnerabilityCount} security vulnerabilities`);
      } else {
